...
def update_branch_protection(repo: OrganizerRepository, branch_name: str):
    ...
    if branch_name not in repo._settings['branches']:
        return
    print(f"Updating branch protection for {branch_name} in {repo.organization.login}/{repo.name}.")
    ...
    repo.branch_protection(
        ...
        required_conversation_resolution=bsettings.get('required_conversation_resolution', NotSet),
